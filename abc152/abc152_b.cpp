...
int main(){
    ...
    cin >> buf_answer;
    var_b = buf_answer;
    for (int cnt_0=0;cnt_0<var_b.asNumber();cnt_0++){
        var_B = Var(var_B.asString() + var_b.asString());
    }
    for (int cnt_1=0;cnt_1<var_a.asNumber();cnt_1++){
        var_A = Var(var_A.asString() + var_a.asString());
    }
    if ((letterOf(Var(1), var_A) < letterOf(Var(1), var_B))){
        cout << var_A << endl;
    } else {
        if ((letterOf(Var(1), var_B) < letterOf(Var(1), var_A))){
            cout << var_B << endl;
        } else {
        }
    }
