...
Var var_K(7);
Var var_S("nikoandsolstice");
Var var_i(7);
Var var_ans("0nikoand");
...
int main(){
    ...
    cin >> buf_answer;
    var_S = buf_answer;
    if ((Var(var_S.asString().size()) < (var_K + Var(1)))){
        cout << var_S << endl;
    } else {
        for (int cnt_0=0;cnt_0<var_K.asNumber();cnt_0++){
            var_i += Var(1);
            var_ans = Var(var_ans.asString() + letterOf(var_i, var_S).asString());
        }
        cout << Var(var_ans.asString() + Var("...").asString()) << endl;
    }
    return 0;
}
