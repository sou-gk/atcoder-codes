...
int main(){
    ...
    cin >> buf_answer;
    var_S = buf_answer;
    for (int cnt_1=0;cnt_1<var_N.asNumber();cnt_1++){
        var_i += Var(1);
        for (int cnt_0=0;cnt_0<Var(2).asNumber();cnt_0++){
            var_result = Var(var_result.asString() + letterOf(var_i, var_S).asString());
        }
    }
